...
import { Validator, FileInput, ConsoleOutput } from 'seo-validator'
...
urlStudy.forEach(site => {
  console.log(`Starting SEO analysis for ${site}`)
  configuration.protocol = site.startsWith('https://') ? 'https:' : 'http:'
  let urlParts = URLParse(site)
  configuration.host = urlParts.hostname
  configuration.startPage = urlParts.pathname
...
//  const input = new UrlInput(url)
  const output = new ConsoleOutput()

  let links = []
  urlStudy.forEach(async (site) => {
    const response = await fetch(site)
    const html = await response.text()
    const $ = cheerio.load(html)
    console.log(`SEO Analysis for: ${site}\n`)
    $("li a").each((idx, el) => {
      const cheeriofied = $(el)
      const output = `${idx + 1}.\t${cheeriofied.text()}`
      links.push(cheeriofied.attr('href'))
    })
  })
  links.forEach(async (link) => {
    console.log(`Scanning link ${link} for useful information`)

    const linkResponse = await fetch(link)
    const linkHtml = await linkResponse.text()
    validator.addInput(new FileInput(linkHtml))
  })
}
